...
for item in parsed_csv:
    ...
    if not psgc_item:
        print('Error matching:', item)
        continue
    
    pop = item['population']
    pct = round((pop / total_pop) * 100, 2)
    ...
    barangays.append({
        ...
        'name': clean_name,
        'official_psgc_name': psgc_item['name'],
        'barangay_number': num,
        'population': pop,
        'percentage': pct,
        'urban_rural': 'Urban' if 'Pob' in clean_name or 'Pob' in psgc_item['name'] or pop >= 5000 else 'Rural'
    })

barangays_sorted = sorted(barangays, key=lambda x: x['barangay_number'])

output_json = {
    '_schema_version': '1.0',
    '_source': 'Philippine Statistics Authority (PSA) - 2024 Census of Population & PSGC',
    'city': 'City of Legazpi',
    'province': 'Albay',
    'region': 'Region V - Bicol Region',
    'city_code': '050506000',
    'psgc_10digit': '0500506000',
    'total_population': total_pop,
    'barangay_count': len(barangays),
    'census_year': 2024,
    'data': barangays_sorted
}

with open('data/barangays.json', 'w', encoding='utf-8') as f:
    json.dump(output_json, f, indent=2, ensure_ascii=False)

print(f'Successfully generated data/barangays.json with {len(barangays)} barangays, total population = {sum(b["population"] for b in barangays):,}.')
