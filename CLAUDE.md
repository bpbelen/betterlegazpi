...
This repo is a **fork-in-progress of BetterSolano.org being retrofitted for LGU Legazpi City** ("BetterLegazpi.org"). Rebranding is underway: `package.json` is now named `betterlegazpi`, the PWA shell precaches `assets/images/logo/betterlegazpi-logo.png`, and "Solano" survives in `README.md`, several docs, and historical page notes. An occurrence of "Solano" may be a genuine leftover _or_ a deliberate reference to the upstream sibling project — check before "fixing" it, and don't assume every occurrence is a bug.
